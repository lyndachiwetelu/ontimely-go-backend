...
func HandleGoogleAuthorizeCalendar(ctx *gin.Context) {
	...
	log.Printf("idUUID sending %s",calendarID.String())
	ctx.Redirect(302, fmt.Sprintf("%s/user/dashboard/calendar/connected?rd=%s", appUrl, url.QueryEscape(calendarIDEncrypted)))
}
...
func GetCalendarInformation(ctx *gin.Context, calendarInfo userCalendarResponseInfoDTO) *uuid.UUID {
	...
	if err != nil {
		log.Printf("Unable to check if Primary Calendar for user exists, error occurred %v, cal: %v", err, calendarExists)
	}
